...
def main(base_dir):
	...
	sub_paths = sorted(glob(join(base_dir, '*')))
	#print "Subjects:", sub_paths

	for sub in sub_paths:
		#print "SUB: ", sub
		seq_paths = sorted(glob(join(sub, '*')))
		#print "Sequence: ", seq_paths

		for seq in seq_paths:
			ang_paths = sorted(glob(join(seq, '*')))
			#print "Angles: ", ang_paths

			for ang in ang_paths:
			#	print "ANGLE:", ang
				align_all(ang)
				
def align_all(base_dir):#, out_dir):
	""" Align silhouettes
	...
	if not image_paths == []:
		for i_path in image_paths:	
			...
			images.append(align_images(base_dir, i_path, pad_image(i_path)))#, out_dir))
...
def align_images(base_path, image_path, image):#, out_dir):
	...
	for y1 in range(0, height - 1):
		for x1 in range(0, width-1):
			if img[((height-1)-y1),((width-1)-x1)][0] > 0:
				bottom = [((width-1)-x1),((height-1)-y1)]
				print "Bottom:",bottom
				break
		else:
			continue 
